...
public class CMD_IClaim implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player)
        {
            ...
            PlayerData playerData = GriefPrevention.perPlayerInstances.get(player.getUniqueId());
            if (args.length == 1) {
                //================================================================================
                // Commands for getting the Wand/Tool
                //================================================================================
                if (args[0].equalsIgnoreCase("tool"))           player.getInventory().addItem(GriefPrevention.iWand);
                if (args[0].equalsIgnoreCase("wand"))           player.getInventory().addItem(GriefPrevention.iWand);
                ...
                if (args[0].equalsIgnoreCase("add"))
                {
                    ...
                    {
                        ResultSet rs = GriefPrevention.DATABASE.executeQuery("SELECT * FROM ?PRE?_claims WHERE claimUUID='" + playerData.currentClaimEdit + "'");
                        try {
                            rs.last();
                            ...
                            GriefPrevention.DATABASE.executeQuery(SQL);
            ...
            if (args.length > 1)
            {
                ...
                if (args[0].equalsIgnoreCase("create"))
                {
                    try
                    {
                        ...
                        String sql = "SELECT * FROM ?PRE?_players WHERE UUID='%s'".formatted(player.getUniqueId());
                        ResultSet res = GriefPrevention.DATABASE.executeQuery(sql);
                        res.last();
                        ...
                        ResultSet res1 = GriefPrevention.DATABASE.executeQuery("SELECT * FROM ?PRE?_ICMs WHERE Name='%s'".formatted(args[1]));
                        res1.last();
                        ...
                        //create the claim
                        GriefPrevention.DATABASE.executeQuery("INSERT INTO ?PRE?_claims(playerUUID, claimUUID, claimName, claimSize) VALUES('%s', '%s', '%s', 0)".formatted(pid, UUID.randomUUID().toString(), args[1]));
                    }
                ...
                if (args[0].equalsIgnoreCase("edit"))
                {
                    try {
                        ...
                        ResultSet res1 = GriefPrevention.DATABASE.executeQuery("SELECT playerUUID FROM ?PRE?_players WHERE playerID=%s".formatted(pid));
                        res1.last();
